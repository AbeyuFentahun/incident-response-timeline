    ...
    try:
        ...
        for folder in folders:
            ...
            if response.get("Contents"):
                ...
                logger.info(f"Folder already exists: s3://{S3_BUCKET}/{folder} "
                ...
                logger.info(f"Created folder: s3://{S3_BUCKET}/{folder} "
                            f"(ENV={os.getenv('ENVIRONMENT', 'local')}, REGION={os.getenv('AWS_REGION')})."
                    )
        # Logs succesful verificaiton or creation to the log file
        logger.info(f"S3 folder structure verified/created in bucket '{S3_BUCKET}'. "
                    f"(ENV={os.getenv('ENVIRONMENT', 'local')}, REGION={os.getenv('AWS_REGION')})."
            )
